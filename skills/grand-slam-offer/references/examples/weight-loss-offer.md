# Complete Example: Weight Loss Grand Slam Offer

From Alex Hormozi's $100M Offers book (Chapters 8-9, 14-15). This is the most detailed example in the book, showing every step of the transformation.

<overview>

**Before:** $99/month bootcamp membership nobody would buy ("LA Fitness is $29/mo")

**After:** $599-$5,200 offer people eagerly purchased (2 refunds out of 4,000+ sales)

</overview>

<dream_outcome>

**The shift:** Sell the destination, not the vehicle.

| Before (Process) | After (Destination) |
|------------------|---------------------|
| "6-week fitness program" | "Lose 20lbs in 6 weeks" |
| "Bootcamp membership" | "Get the body your spouse can't keep their hands off" |

**Final Dream Outcome:** "Lose 20lbs in 6 weeks"
- Specific (20lbs)
- Time-bound (6 weeks)
- Visceral (they can feel wanting it)
- Status-enhancing (others will notice)

</dream_outcome>

<problem_list>

**Target: 32-64 problems across 4 categories per action**

### Action 1: Buy Healthy Food

| Category | Problems |
|----------|----------|
| Likelihood | "I don't know what's actually healthy" / "My family won't eat this" / "I'll mess it up" |
| Time | "Grocery shopping takes forever" / "Too busy for multiple stores" |
| Effort | "Reading labels is confusing" / "Don't know how to find deals" |
| Sacrifice | "Healthy food is expensive" / "I'll miss my comfort foods" |

### Action 2: Cook Healthy Food

| Category | Problems |
|----------|----------|
| Likelihood | "I'm a terrible cook" / "My meals never taste good" / "I'll get bored" |
| Time | "Cooking takes too long" / "No time for meal prep" |
| Effort | "Recipes are complicated" / "Don't know kitchen basics" |
| Sacrifice | "Have to give up eating out" / "No more quick takeout" |

### Action 3: Eat Healthy Food Consistently

| Category | Problems |
|----------|----------|
| Likelihood | "I always fall off diets" / "No willpower" / "Social events derail me" |
| Time | "Eating healthy takes too long" / "Can't meal prep every week" |
| Effort | "Tracking calories is tedious" / "Portions are confusing" |
| Sacrifice | "I'll miss happy hour" / "Can't enjoy holidays" |

### Action 4: Exercise Regularly

| Category | Problems |
|----------|----------|
| Likelihood | "I hate the gym" / "Don't know proper form" / "I'll hurt myself" |
| Time | "Don't have an hour daily" / "Commute to gym is too long" |
| Effort | "Workouts are confusing" / "Don't know what to do" |
| Sacrifice | "I'll be sore" / "I'll miss my TV time" |

**Total: 40+ problems identified**

</problem_list>

<solutions>

**Formula:** "How to [reverse the problem] (even if [their excuse])"

| Problem | Solution |
|---------|----------|
| "Don't know what's healthy" | "How to know exactly what to buy (no nutrition degree required)" |
| "Grocery shopping takes forever" | "How to complete healthy shopping in under 20 minutes" |
| "I'm a terrible cook" | "How anyone can make delicious meals (even if you burn toast)" |
| "Cooking takes too long" | "How to cook healthy meals in under 5 minutes" |
| "I always fall off diets" | "How to stay consistent (even when life gets crazy)" |
| "I hate the gym" | "How to get better results than gym-goers without a gym" |
| "I'll miss happy hour" | "How to socialize without derailing your progress" |

</solutions>

<value_stack>

**The complete offer exactly as presented in Chapter 9:**

```
═══════════════════════════════════════════════════════════════
    FREE SIX-WEEK LEAN-BY-HALLOWEEN CHALLENGE
═══════════════════════════════════════════════════════════════

CORE OFFER: Lose 20lbs in 6 Weeks

WHAT'S INCLUDED:

✓ Foolproof Bargain Grocery System
  "...saves hundreds per month, takes less time than current routine"
  ──────────────────────────────────────────── $1,000 value

✓ "Ready in 5 min" Busy Parent Cooking Guide
  "...how anyone can eat healthy even if they have no time"
  ──────────────────────────────────────────── $600 value

✓ Personalized "Lick Your Fingers Good" Meal Plan
  "...so good it'll be easier than your 'cheat' foods"
  ──────────────────────────────────────────── $500 value

✓ Fat Burning Workouts (Better Than Doing It Alone)
  "...adjusted to your needs, no plateau, no injury risk"
  ──────────────────────────────────────────── $699 value

✓ The "Never Fall Off" Accountability System
  "...works without your permission"
  ──────────────────────────────────────────── $1,000 value

✓ "Live It Up While Slimming Down" Eating Out System
  "...freedom to eat out without feeling like the odd man out"
  ──────────────────────────────────────────── $349 value

✓ "Tone Up While You Travel" Eating & Workout Blueprint
  "...amazing workouts with no equipment"
  ──────────────────────────────────────────── $199 value

═══════════════════════════════════════════════════════════════
              TOTAL VALUE: $4,351
              YOUR INVESTMENT: $599
═══════════════════════════════════════════════════════════════
```

**Note:** "Most of our facilities now sell this for $2,400 to $5,200." — Hormozi

</value_stack>

<guarantee>

**Hormozi's exact script (Chapter 14):**

> "Do you think I'd still be in business if I gave a crazy guarantee like that and wasn't good at what I did? Now I'm not guaranteeing you're going to hit this goal in six weeks, after all, because I can't eat the food for you. But I am guaranteeing that you will get $500 worth of value and service from us to support you. If you don't feel like we gave you that level of service, I'll write you a check the day you tell me we suck."

**The close:**

> "Best case you get the body of your dreams and we give you all your money towards staying with us to hit your long-term goal. Worst case you tell me I suck, I write you a check, and you get six weeks of free training. Both options are risk free. But, the only thing guaranteed not to help you is walking out of here today."

**Result:** 2 refunds out of 4,000+ sales in 3.5 years.

</guarantee>

<naming>

**M.A.G.I.C. examples from the book (Chapter 15):**

- "Free Six-Week Lean-By-Halloween Challenge"
- "88% Off 12-Week Bikini Blueprint"
- "Free 21-Day Mommy Makeover"

**Components used:**
- **M** (Magnet): "Free" or "88% Off"
- **A** (Avatar): "Lakeway Moms" (local area), "Busy Parent"
- **G** (Goal): "Lean," "Bikini," "Mommy Makeover"
- **I** (Interval): "6-Week," "12-Week," "21-Day"
- **C** (Container): "Challenge," "Blueprint"

The core offer stayed the same—only the name wrapper rotated seasonally.

</naming>

<results>

| Metric | Before | After |
|--------|--------|-------|
| Price | $99/month (couldn't sell) | $599 → $2,400-$5,200 |
| Comparison | "LA Fitness is $29/mo" | Category of one |
| Refunds | N/A | 2 out of 4,000+ |
| Perception | "Too expensive" | Eager buyers |

</results>

<why_it_works>

**Value Equation Analysis:**

| Driver | How Maximized |
|--------|---------------|
| Dream Outcome | Specific "20lbs in 6 weeks" not vague "get fit" |
| Likelihood | Accountability system + personalization + proof |
| Time Delay | 6-week timeline with quick wins built in |
| Effort | Done-for-you meal plans, 5-min cooking, no gym required |

**The transformation:** Same service delivery, completely different positioning and packaging.

</why_it_works>
